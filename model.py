...
class MaskRCNN(nn.Module):
    ...
    def evaluate_map(self, test_dataset, image_list, vocabulary):
        ...
        for i_data, data in enumerate(val_generator):
            image = image_list[i_data]
            gt_class_ids = data[4].squeeze(0).numpy()
            gt_boxes = data[5].squeeze(0).numpy()
            gt_masks = data[6].squeeze(0).numpy()
            ...
            result = self.detect([image])[0]
            print('detected {}'.format(i_data))
            detection_results.append(result)

        assert len(test_roidb) == len(image_list)

        # get all_boxes_gt from detection results
        num_images = len(detection_results)
